...
const main = async () => {
  ...
  try {
    ...
    await runCommand('npm', ['run', 'build'], {
      env: { VITE_API_URL: `${apiBaseUrl}/api`, VITE_POSTBAC_AUTO_V1_ENABLED: 'true' },
    });
    ...
    const primary = await registerVerifyLogin({
      apiBaseUrl,
      verificationTokens,
      email: `postbac-primary-${suffix}@example.test`,
      password,
    });
    ...
    const secondary = await registerVerifyLogin({
      apiBaseUrl,
      verificationTokens,
      email: `postbac-secondary-${suffix}@example.test`,
      password,
    });
    ...
    // --- Recette post-bac (flag actif) : le resume automatique doit apparaitre ---
    await waitForText(page, 'Profil Makoki des intérêts professionnels');
    const dominantCount = await page.evaluate(
      () => Array.from(document.querySelectorAll('h3')).filter((e) => /Concret et technique|Analyse et recherche|Création et expression|Relation et aide|Initiative et gestion|Organisation et rigueur/.test(e.textContent || '')).length,
    );
    assert.ok(dominantCount >= 3, `3 dimensions dominantes attendues, obtenu ${dominantCount}`);

    await waitForText(page, 'Métiers à explorer');
    const postbacCareerCount = await page.evaluate(
      () => Array.from(document.querySelectorAll('section[aria-labelledby="postbac-careers-title"] a')).filter((l) => /Voir la fiche métier/.test(l.textContent || '')).length,
    );
    assert.ok(postbacCareerCount > 0 && postbacCareerCount <= 6, `<=6 metiers post-bac attendus, obtenu ${postbacCareerCount}`);

    // Les raisons s'appuient sur les descriptions statiques des dimensions.
    const reasonMatches = await page.evaluate(() => {
      const activities = [
        'activités concrètes', "activités d’analyse", 'activités créatives',
        "activités d’aide", "activités d’initiative", "activités d’organisation",
        'proximité d’intérêts',
      ];
      const text = document.body.innerText;
      return activities.some((a) => text.includes(a));
    });
    assert.ok(reasonMatches, 'les raisons doivent utiliser le vocabulaire des dimensions');

    // Aucune promesse interdite dans le resume.
    const forbidden = await page.evaluate(() => {
      const text = document.body.innerText.toLowerCase();
      return [
        'vous réussirez', 'fait pour vous', 'aptitudes nécessaires',
        'de chances', 'métier idéal', 'avenir garanti',
        'garantie d’emploi', 'garantie d’admission', 'probabilité de réussite',
      ].filter((p) => text.includes(p));
    });
    assert.deepEqual(forbidden, [], `promesses interdites detectees: ${forbidden.join(', ')}`);

    // CTA conseiller sans canal configure -> etat honnete.
    await waitForText(page, 'Accompagnement en préparation');
    ...
    console.log('POSTBAC FUNCTIONAL E2E PASSED');
  } finally {
