...
type StepKey = 'identity' | 'objective' | 'education' | 'skills' | 'summary';
...
export default function AdaptiveProfileWizard() {
  ...
  const steps = useMemo<StepDefinition[]>(() => [
    { key: 'identity', label: 'À propos de toi', description: 'Tes informations essentielles' },
    { key: 'objective', label: 'Projet', description: 'Ta situation et ton objectif' },
    ...(showEducation
      ? [{ key: 'education' as const, label: 'Études', description: 'Ton parcours scolaire et tes formations' }]
      : []),
    { key: 'skills', label: 'Compétences', description: 'Ce que tu sais déjà faire' },
    { key: 'summary', label: 'Résumé', description: 'Relis et finalise ton profil' },
  ], [showEducation]);
  ...
  useEffect(() => {
    void getAdaptiveProfile()
      ...
      .catch((error) => {
        ...
        toast.error('Impossible de charger ton profil. Reconnecte-toi puis réessaie.');
      })
      .finally(() => setLoading(false));
    // Le brouillon local n'est lu qu'au montage.
    ...
    const timer = window.setTimeout(() => {
      ...
      void searchEscoSkills(query, { locale: 'fr', limit: 8, signal: controller.signal })
        ...
        .catch((error) => {
          ...
          setSkillSearchError('La recherche de compétences est indisponible. Tu peux ajouter ta compétence manuellement.');
          setSkillSuggestions([]);
        })
        .finally(() => {
  ...
  const finish = async () => {
    ...
    try {
      ...
      toast.success('Ton profil est à jour.');
    } catch (error) {
      console.error('Unable to finish adaptive profile', error);
      toast.error("Impossible d'enregistrer ton profil.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const addSkill = (suggestion: EscoSkillSuggestion) => {
    if (skills.some((skill) => skill.esco_uri === suggestion.esco_uri)) {
      toast.info('Cette compétence est déjà dans ton profil.');
      return;
    }
  ...
  if (loading) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center" aria-label="Chargement du profil">
    ...
    <div className="space-y-6">
      {resumedFromDraft ? (
        <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
          <Save className="h-4 w-4 shrink-0" /> Ta saisie non terminée a été restaurée sur cet appareil.
        </div>
      ) : null}
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h2 className="flex items-center gap-2 text-2xl font-semibold leading-none tracking-tight">
                  <Sparkles className="h-6 w-6 text-blue-600" /> Compléter mon profil
                </h2>
                <CardDescription className="mt-2 max-w-2xl">
                  Renseigne seulement les informations utiles à ton projet. Tu peux revenir les modifier à tout moment.
                </CardDescription>
              </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Ton compte</CardTitle>
            <CardDescription>Espace personnel</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            <p className="truncate font-medium">{storedUser?.email || 'Session active'}</p>
            <p className="text-gray-500">Tes informations sont enregistrées dans ton espace.</p>
          </CardContent>
        </Card>
      </div>

      <ol aria-label="Progression du profil" className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
        {steps.map((step, index) => (
          <li
            key={step.key}
            aria-current={index === activeStep ? 'step' : undefined}
            className={`rounded-lg border p-3 text-sm ${index === activeStep ? 'border-blue-500 bg-blue-50' : index < activeStep ? 'border-emerald-200 bg-emerald-50' : 'bg-white'}`}
          >
            <span className="flex items-center gap-2 font-semibold">
              <span className={`flex h-6 w-6 items-center justify-center rounded-full text-xs ${index <= activeStep ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
      ...
      <Card aria-labelledby="profile-step-title">
        <CardHeader>
          <CardTitle id="profile-step-title">{currentStep?.label}</CardTitle>
        ...
        <CardContent className="space-y-6">
            ...
            <section className="space-y-5">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">Utilisé uniquement pour te recontacter si nécessaire.</p>
                </div>
                <div>
              ...
              <div className="grid gap-4 md:grid-cols-[1fr_140px]">
                ...
                <div>
                  ...
                  <Input id="country_code" maxLength={2} value={profile.country_code || ''} onChange={(event) => updateProfile('country_code', event.target.value.toUpperCase() || null)} />
                  <p className="mt-1 text-xs text-gray-500">Ex. CG pour Congo</p>
                </div>
              </div>
            </section>
          ) : null}
          ...
          {currentStep?.key === 'objective' ? (
            <section className="space-y-5">
              <div>
                <Label htmlFor="current_situation">Situation actuelle</Label>
              ...
              <div>
                <Label htmlFor="primary_goal">Objectif prioritaire</Label>
              ...
              <div>
                <Label htmlFor="mobility_scope">Mobilité</Label>
              ...
              {!showEducation ? (
                <p className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                  Cette étape n’est pas nécessaire pour ta situation. Les formations déjà enregistrées restent disponibles.
                </p>
              ) : null}
            </section>
          ) : null}
            ...
            <section className="space-y-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Étape facultative, modifiable à tout moment.</p>
                </div>
                ...
                <div key={entry.id || index} className="space-y-4 rounded-xl border p-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div>
                      ...
                      <select id={`level-${index}`} className={selectClassName} value={entry.education_level} onChange={(event) => updateEducation(index, { education_level: event.target.value as EducationRecord['education_level'] })}>
                    ...
                    <div>
                      ...
                      <select id={`status-${index}`} className={selectClassName} value={entry.status} onChange={(event) => updateEducation(index, { status: event.target.value as EducationRecord['status'] })}>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div>
                      <Label htmlFor={`diploma-${index}`}>Diplôme</Label>
                      <Input id={`diploma-${index}`} value={entry.diploma_name || ''} onChange={(event) => updateEducation(index, { diploma_name: event.target.value || null })} />
                    </div>
                    <div>
                      <Label htmlFor={`field-${index}`}>Domaine</Label>
                      <Input id={`field-${index}`} value={entry.field_of_study || ''} onChange={(event) => updateEducation(index, { field_of_study: event.target.value || null })} />
                    </div>
                  </div>
                  <div>
                    <Label htmlFor={`institution-${index}`}>Établissement</Label>
                    <Input id={`institution-${index}`} value={entry.institution || ''} onChange={(event) => updateEducation(index, { institution: event.target.value || null })} />
                  </div>
                  <div className="grid gap-4 md:grid-cols-3">
                    <div>
                      <Label htmlFor={`start-${index}`}>Début</Label>
                      <Input id={`start-${index}`} inputMode="numeric" value={entry.start_year || ''} onChange={(event) => updateEducation(index, { start_year: event.target.value ? Number(event.target.value) : null })} />
                    </div>
                    <div>
                      <Label htmlFor={`end-${index}`}>Fin</Label>
                      <Input id={`end-${index}`} inputMode="numeric" value={entry.end_year || ''} onChange={(event) => updateEducation(index, { end_year: event.target.value ? Number(event.target.value) : null })} />
                    </div>
                    <div>
                      <Label htmlFor={`country-${index}`}>Pays</Label>
                      <Input id={`country-${index}`} maxLength={2} value={entry.country_code || ''} onChange={(event) => updateEducation(index, { country_code: event.target.value.toUpperCase() || null })} />
                    </div>
                  </div>
                </div>
              ))}
            ...
            <section className="space-y-6">
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <Input id="skill-search" type="search" className="pl-9" value={skillQuery} onChange={(event) => setSkillQuery(event.target.value)} placeholder="Ex. analyser des données…" />
                  {searchingSkills ? <Loader2 className="absolute right-3 top-3 h-4 w-4 animate-spin" /> : null}
                </div>
                <p className="mt-2 text-xs text-gray-500">Choisis une suggestion ou ajoute ta propre formulation.</p>
                <div aria-live="polite">
                  {skillSearchError ? <p className="mt-2 text-sm text-amber-700">{skillSearchError}</p> : null}
                </div>

                {skillSuggestions.length > 0 ? (
                  <ul aria-label="Suggestions de compétences" className="mt-2 max-h-72 overflow-auto rounded-lg border bg-white shadow-lg">
                    {skillSuggestions.map((suggestion) => (
                      <li key={suggestion.id} className="border-b last:border-b-0">
                ...
                {skillQuery.trim().length >= 2 && !searchingSkills ? (
                  <Button type="button" variant="ghost" size="sm" className="mt-2" onClick={addManualSkill}>
                    <Plus className="mr-2 h-4 w-4" /> Ajouter « {skillQuery.trim()} »
                  </Button>
                ) : null}
              </div>

              <div className="space-y-3">
                <h3 className="font-semibold">Compétences ajoutées</h3>
                {!declaredSkills.length ? (
                  <p className="rounded-lg border border-dashed p-5 text-sm text-gray-500">Ajoute une compétence que tu utilises déjà.</p>
                ) : null}
                {declaredSkills.map((skill, index) => (
                  <div key={skill.id || skill.esco_uri || `${skill.label}-${index}`} className="grid items-center gap-3 rounded-lg border p-3 md:grid-cols-[1fr_180px_auto]">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{skill.esco_uri ? 'Issue du catalogue de compétences' : 'Ajoutée manuellement'}</p>
                    </div>
                    <select
                    ...
                    <Button type="button" variant="ghost" size="icon" aria-label={`Retirer ${skill.label}`} onClick={() => removeSkill(skill)}>
              ...
              {sourcedSkills.length > 0 ? (
                <div className="space-y-3">
                  <h3 className="font-semibold">Compétences repérées dans ton parcours</h3>
            ...
            <section className="space-y-5">
              <div className="grid gap-3 md:grid-cols-2">
                <div className="rounded-lg border p-4">
                  <p className="text-xs uppercase text-gray-500">Situation</p>
                  <p className="mt-1 font-semibold">{profile.current_situation ? situationLabels[profile.current_situation] : 'À préciser'}</p>
                </div>
                <div className="rounded-lg border p-4">
                  <p className="text-xs uppercase text-gray-500">Objectif</p>
                  <p className="mt-1 font-semibold">{profile.primary_goal ? goalLabels[profile.primary_goal] : 'À préciser'}</p>
                </div>
                <div className="rounded-lg border p-4">
                  <p className="text-xs uppercase text-gray-500">Formations</p>
                  <p className="mt-1 font-semibold">{education.length}</p>
                </div>
                <div className="rounded-lg border p-4">
                  <p className="text-xs uppercase text-gray-500">Compétences</p>
                  <p className="mt-1 font-semibold">{skills.filter((skill) => skill.confirmation_status !== 'rejected').length}</p>
                </div>
              </div>
              <div>
                <Label htmlFor="profile_summary">Ton projet en quelques phrases <span className="text-gray-500">(facultatif)</span></Label>
                <Textarea id="profile_summary" rows={6} value={profile.profile_summary || ''} onChange={(event) => updateProfile('profile_summary', event.target.value || null)} />
              </div>
              <p className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                Les informations que tu déclares et les résultats de ton parcours restent présentés séparément.
              </p>
            </section>
          ) : null}

          <div className="flex flex-wrap items-center justify-between gap-3 border-t pt-5">
            <Button type="button" variant="outline" onClick={() => void moveTo(activeStep - 1)} disabled={activeStep === 0 || saving}>
              <ChevronLeft className="mr-2 h-4 w-4" /> Précédent
            </Button>
            {activeStep < steps.length - 1 ? (
              <Button type="button" onClick={() => void moveTo(activeStep + 1)} disabled={saving}>
                {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                {currentStep?.key === 'education' && !education.length ? 'Passer et continuer' : 'Enregistrer et continuer'}
