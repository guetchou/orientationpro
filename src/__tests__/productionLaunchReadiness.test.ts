...
const expectedSha = 'b281afc69296aac3af084edaa4c30b550f57e457';
...
describe('production launch readiness', () => {
  it('serves the final release, launch services, capability state and MAKOKI identity', async () => {
    const health = await waitForExpectedProduction();
    expect(health.gitSha).toBe(expectedSha);

    for (const path of [
      '/', '/parcours', '/careers', '/jobs', '/cv-optimizer', '/recruitment',
      '/conseiller', '/legal', '/cookies', '/sitemap.xml',
    ]) {
