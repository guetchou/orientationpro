...
Observation : 2026-07-29. Base : `de5a298` (#103, #121 et #123 intégrées).
Révision obligatoire après chaque fusion V5 modifiant une frontière de sécurité.
...
| SEC-002 | fait + risque résiduel | Limiteurs en mémoire raccordés ; charge réelle et coordination multi-instance non prouvées | `rate-limit.js`, `rate-limit-security.test.js`, `server.js` sur `de5a298` | haute | mainteneur V5 | avant fusion #112 | compensé par #123, non suffisant pour GO | charge métier, métriques et stratégie multi-instance |
| SEC-003 | fait vérifié | Logs directs ; sérialisation allowlist non raccordée au runtime | `backend/src/server.js`, audit sur `de5a298` | critique | lot #124 | avant fusion #124 | bloquant, non accepté | logger raccordé et canaris |
...
| SEC-006 | inconnue | Isolation CPU/temps/mémoire du parseur document non démontrée | aucune preuve d’exécution identifiée sur `de5a298` | haute | mainteneur V5 | avant fusion #112 | bloquant avant public | limites et essais réels documentés |
