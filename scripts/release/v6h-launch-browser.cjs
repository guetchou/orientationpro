...
const outputPath = process.env.V6H_BROWSER_OUTPUT || '/tmp/v6h-launch-browser.json';
const screenshotPath = process.env.V6H_SCREENSHOT_PATH || '/tmp/v6h-launch-parcours.png';
const pdfPath = process.env.V6H_PDF_PATH || '/tmp/v6h-launch-synthesis.pdf';
...
const completeGuestQuestionnaire = async (page) => {
  ...
  if (!(await intro.innerText()).includes('60')) {
    throw new Error('The public instrument does not announce 60 affirmations.');
  }
  ...
  for (let index = 0; index < 60; index += 1) {
    ...
    await answers.nth(index % 5).click();
    if (index === 59) {
  ...
  if (await guestPreview.getByRole('link').count() !== 0) {
    throw new Error('The guest preview exposes a navigation link.');
  }
...
const fillDiagnosticAndGenerate = async (page) => {
  ...
  await page.getByLabel('Diplôme principal').fill('Baccalauréat obtenu');
  ...
  await page.getByLabel('Expériences utiles').fill('Projet scolaire avec tableur et aide numérique informelle');
  await page.getByLabel('Contraintes importantes').fill('Transport local, budget limité, informations vérifiables');
  ...
  const comparison = page.locator('[aria-labelledby="life-project-comparison-title"]');
  await comparison.waitFor({ timeout: 60000 });
  const comparisonText = await comparison.innerText();
  for (const heading of ['Option', 'Durée', 'Coût', 'Calendrier', 'Modalités', 'Accès / zone']) {
    if (!comparisonText.includes(heading)) throw new Error(`Comparison is missing ${heading}.`);
  }

  const summary = page.locator('#life-project-summary');
  await summary.waitFor({ timeout: 60000 });
  const summaryText = await summary.innerText();
  if (!summaryText.includes('Piste retenue provisoirement') || !summaryText.includes('Première action à réaliser')) {
    throw new Error('The final synthesis is incomplete.');
  }
  ...
  return {
    ...
    printableSynthesisVisible: true,
  };
};
...
const main = async () => {
  ...
  try {
    firstContext = await browser.newContext({ locale: 'fr-FR', viewport: { width: 1440, height: 1100 }, serviceWorkers: 'block' });
    ...
    await firstPage.locator('[aria-labelledby="life-project-comparison-title"]').waitFor({ timeout: 60000 });
    await firstPage.locator('#life-project-summary').waitFor({ timeout: 60000 });
    const persistenceAfterReload = true;

    await firstContext.close();
    firstContext = null;

    secondContext = await browser.newContext({ locale: 'fr-FR', viewport: { width: 1440, height: 1100 }, serviceWorkers: 'block' });
    ...
    await secondPage.locator('[aria-labelledby="life-project-comparison-title"]').waitFor({ timeout: 60000 });
    await secondPage.locator('#life-project-summary').waitFor({ timeout: 60000 });

    await secondPage.screenshot({ path: screenshotPath, fullPage: true });
    await secondPage.emulateMedia({ media: 'print' });
    const printedComparisonVisible = await secondPage.locator('[aria-labelledby="life-project-comparison-title"]').isVisible();
    if (!printedComparisonVisible) throw new Error('The complete comparison is hidden in print mode.');
    ...
    if (diagnostics.pageErrors.length > 0) throw new Error(`Browser page errors: ${diagnostics.pageErrors.join(' | ')}`);
    if (diagnostics.serverErrors.length > 0) throw new Error(`HTTP 5xx responses: ${diagnostics.serverErrors.join(' | ')}`);
  } finally {
...
main().catch((error) => {
  process.stderr.write(`V6-H launch browser recipe failed: ${error.message}\n`);
  process.exitCode = 1;
});
