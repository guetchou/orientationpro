...
echo "🔑 Configuration API Key N8N et activation des workflows"

# Configuration
N8N_URL="http://localhost:5678"
API_KEY="orientationpro-n8n-api-key-$(date +%s)"

echo -e "${BLUE}🔑 Génération de la clé API: $API_KEY${NC}"
...
setup_api_key() {
    ...
    docker stop n8n-orientationpro 2>/dev/null
    docker rm n8n-orientationpro 2>/dev/null
    
    # Modifier le docker-compose.yml pour ajouter l'API key
    echo "📝 Modification du docker-compose.yml..."
    sed -i "s/N8N_ENCRYPTION_KEY=.*/N8N_ENCRYPTION_KEY=$API_KEY/" /opt/n8n/docker-compose.yml
    
    # Ajouter la variable API key si elle n'existe pas
