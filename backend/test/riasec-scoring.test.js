...
test('returns a unique three-letter code only when the first three ranks are unambiguous', () => {
  const items = createItems(Object.fromEntries(dimensions.map((dimension) => [dimension, 1])));
...
test('reports leading ties instead of breaking them through array order', () => {
  const items = createItems(Object.fromEntries(dimensions.map((dimension) => [dimension, 1])));
  ...
  assert.deepEqual(result.ranking.tieGroups[0].dimensions, ['I', 'R']);
});
...
test('rejects incomplete, duplicate, unknown and out-of-range responses', () => {
  const items = createItems(Object.fromEntries(dimensions.map((dimension) => [dimension, 1])));
