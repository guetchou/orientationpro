...
const occupation = (id, presentationOccupationId, jobZone = 3) => ({
  id,
...
test('education readiness treats Job Zone as a cautious preparation reference', () => {
  const ready = educationReadiness({ education: [{ education_level: 'licence', status: 'completed' }], jobZone: 4 });
  assert.equal(ready.score, 100);
  assert.equal(ready.status, 'meets_reference');

  const inProgress = educationReadiness({ education: [{ education_level: 'bac_plus_2', status: 'in_progress' }], jobZone: 4 });
  assert.equal(inProgress.status, 'development_gap');

  const unknown = educationReadiness({ education: [{ education_level: 'master', status: 'completed' }], jobZone: null });
  assert.equal(unknown.score, 50);
  assert.equal(unknown.status, 'unknown_job_zone');
});
...
test('confirmed ESCO evidence can reorder otherwise close RIASEC matches', () => {
  ...
  const recommendations = rankProfileRecommendations({
    ...
    occupationsById,
    profile: { primary_goal: 'find_job', mobility_scope: 'national' },
  ...
  assert.ok(recommendations[0].cautions.some((message) => message.includes('Congo')));
});
...
test('missing profile signals are omitted rather than converted into zero scores', () => {
  ...
  const context = profileRecommendationContext({ profile: null, education: [], confirmedSkills: [] });
