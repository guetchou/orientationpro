...
// Score déterministe 0-100 réparti en 4 composantes. Aucun point de départ
// arbitraire : chaque point est justifié par un signal détecté.
...
const computeScores = ({ normalized, sectionInfo, skills }) => {
  ...
  const structure = structureBreakdown.reduce((sum, item) => sum + (item.ok ? item.points : 0), 0);

  // 2. Clarté du contenu (25)
  const dates = normalized.text.match(DATE_RE) || [];
  const uniqueYears = Array.from(new Set(dates.map(Number)));
  ...
  const contentClarity = contentBreakdown.reduce((sum, item) => sum + (item.ok ? item.points : 0), 0);

  // 3. Impact professionnel (25)
  ...
  const impact = impactBreakdown.reduce((sum, item) => sum + (item.ok ? item.points : 0), 0);

  // 4. Exploitabilité technique (20)
  ...
  const technical = technicalBreakdown.reduce((sum, item) => sum + item.points * (item.ok ? 1 : 0), 0);

  const generalReadiness = clamp(structure + contentClarity + impact + technical, 0, 100);

  return {
    scores: { generalReadiness, structure, contentClarity, impact, technical },
    signals: { verbHits, quantified, datesOrdered, enoughText, uniqueRatio: Number(uniqueRatio.toFixed(3)), skillsCount: skills.length },
    breakdown: { structure: structureBreakdown, contentClarity: contentBreakdown, impact: impactBreakdown, technical: technicalBreakdown },
  };
};
