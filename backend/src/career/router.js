  ...
  if (!allowed) {
    return res.status(403).json({
      error: {
        ...
        message: 'The authenticated account is not allowed to perform this career action.',
      },
    });
  }
  return next();
...
const createCareerRouter = ({ store, authenticate, hasPermission }) => {
  ...
  router.get(
    '/catalog/summary',
    requirePermission(hasPermission, 'career.catalog.read'),
    route(async (req, res) => {
  ...
  router.get(
    ...
    requirePermission(hasPermission, 'career.catalog.read'),
    route(async (req, res) => {
      const occupation = await store.getOccupation({ occupationId: req.params.occupationId });
      if (!occupation) {
        return res.status(404).json({
          error: {
            code: 'OCCUPATION_NOT_FOUND',
            message: 'The requested occupation does not exist in the active catalog.',
          },
        });
      }
  ...
  router.get(
    '/matches/:resultId',
    requirePermission(hasPermission, 'career.match.read_own'),
    route(async (req, res) => {
      ...
      if (!result) {
        return res.status(404).json({
          error: {
            code: 'ORIENTATION_RESULT_NOT_FOUND',
            message: 'The orientation result does not exist for the authenticated account.',
          },
        });
      }
