...
const createCareerRouter = ({
  ...
  if (!store) throw new Error('Career store is required.');
  if ((!publicCatalog || recommendationsEnabled)
    && (typeof authenticate !== 'function' || typeof hasPermission !== 'function')) {
    throw new Error('Career authentication and permission checks are required for protected routes.');
  }
  const router = express.Router();
  const protectedRoute = (permissionId) => [
    authenticate,
    requirePermission(hasPermission, permissionId),
  ];
  const catalogProtection = publicCatalog ? [] : protectedRoute('career.catalog.read');

  router.get('/catalog/summary', ...catalogProtection, route(async (req, res) => {
    const sources = await store.getCatalogSummary();
    return res.status(200).json({ sources });
  }));

  router.get('/occupations', ...catalogProtection, route(async (req, res) => {
    const locale = requestedLocale(req);
    const occupations = await store.searchOccupations({
      query: req.query.q,
      locale,
      riasecOnly: req.query.riasecOnly === 'true',
      includeLocallyExcluded: publicCatalog ? false : req.query.includeLocallyExcluded === 'true',
  ...
  router.get('/occupations/:occupationId', ...catalogProtection, route(async (req, res) => {
    const locale = requestedLocale(req);
    const occupation = await store.getOccupation({ occupationId: req.params.occupationId, locale });
    if (!occupation || (publicCatalog && occupation.localRelevanceStatus === 'excluded')) {
      return res.status(404).json({ error: { code: 'OCCUPATION_NOT_FOUND', message: 'The requested occupation does not exist in the active catalog.' } });
    }
    return res.status(200).json({ requestedLocale: locale, occupation });
  }));

  if (recommendationsEnabled) {
    router.get('/matches/:resultId', ...protectedRoute('career.match.read_own'), route(async (req, res) => {
    ...
    router.get('/recommendations/:resultId', ...protectedRoute('career.match.read_own'), route(async (req, res) => {
    ...
    router.post('/recommendations/:resultId/snapshots', ...protectedRoute('career.match.read_own'), route(async (req, res) => {
    ...
    router.get('/recommendation-snapshots/:snapshotId', ...protectedRoute('career.match.read_own'), route(async (req, res) => {
